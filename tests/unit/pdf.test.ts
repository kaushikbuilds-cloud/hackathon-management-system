...
import { A4, CARD_SIZES, resolveTemplateConfig } from "@/lib/domain/template";
...
describe("team ID card PDF", () => {
  it.each([1, 2, 4, 7])("produces exactly N pages for N=%i members", async (n) => {
    ...
    const pdf = await generateTeamIdCardsPdf(input(members));
  ...
  it("uses consistent portrait card dimensions on every page", async () => {
    for (const cardSize of ["cr80", "a6", "badge4x6"] as const) {
      const pdf = await generateTeamIdCardsPdf(input([member(1, "leader"), member(2), member(3)], { cardSize }));
      const doc = await PDFDocument.load(pdf.bytes);
  ...
  it("handles long names and non-Latin characters without failing", async () => {
    ...
    const pdf = await generateTeamIdCardsPdf(input([m, unsupported]));
