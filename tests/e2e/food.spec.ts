...
import { FORM_SLUG, cardCredentials, creds, signIn } from "./helpers";
...
test("a participant orders food and cannot exceed the free meal limit", async ({ page }) => {
  ...
  const { participantCode, code } = await cardCredentials(leaderEmail);
  await page.goto("/activate");
  await page.getByLabel("Participant ID").fill(participantCode);
  ...
  // A second lunch is refused by the database even if the page allowed it.
  await meals.getByRole("button", { name: "One more Lunch" }).click();
  await meals.getByRole("button", { name: "Place order" }).click();
...
test("the counter moves the order along and the participant sees it is ready", async ({ page, browser }) => {
  ...
  await signIn(participant, leaderEmail, password);
