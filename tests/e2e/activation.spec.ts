...
const PARTICIPANT = "PRT-2026-0003"; // seeded Code Ninjas member without an account
...
test("printing a team's cards issues a one-time activation code per member without an account", async ({ page }) => {
  ...
  try {
    const { rows } = await pool.query("select team_id, id from participants where participant_code = $1", [PARTICIPANT]);
    ...
    expect(byCode["PRT-2026-0001"]).toBeNull(); // leader already has an account
