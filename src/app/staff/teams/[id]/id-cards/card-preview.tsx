...
export function CardPreview({ eventName, organizer, member, teamName, teamCode, config }: {
  ...
  return (
    <div className="mx-auto flex aspect-[54/85.6] w-full max-w-56 flex-col overflow-hidden rounded-lg bg-white text-[#111827] shadow-brutal ring-1 ring-black/10" aria-label={`Card preview for ${member.full_name}`}>
      <div className="px-3 pt-3 pb-2" style={{ background: config.headerColor }}>
        <p className="truncate text-sm font-bold text-ink">{eventName}</p>
        <p className="truncate text-[9px] text-ink-soft">{organizer}</p>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center px-3 pt-2 text-center">
        ...
        {config.showDepartment && member.department && <p className="line-clamp-1 text-[8px] text-muted">{member.department}{member.academic_year && ` · ${member.academic_year}`}</p>}
        <div className="mt-auto mb-2 flex w-full items-end justify-between gap-2 text-left">
          <div className="min-w-0">
            <p className="text-[6px] font-semibold text-muted">PARTICIPANT ID</p>
            <p className="text-[10px] font-bold">{member.participant_code}</p>
            <p className="mt-0.5 text-[6px] font-semibold text-muted">TEAM</p>
